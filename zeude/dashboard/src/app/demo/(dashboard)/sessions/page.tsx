...
import { getDemoSessionsToday } from '@/lib/demo-mock'
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">Browse your Claude Code sessions (mock dataset)</p>
      </div>
              ...
              <TableBody>
                {sessions.map((session) => (
                  <TableRow key={session.session_id}>
                    ...
                    <TableCell>
                      <Badge variant="secondary">{formatDuration(session.started_at, session.ended_at)}</Badge>
                    </TableCell>
                    ...
                    <TableCell className="text-right font-mono">${Number(session.total_cost || 0).toFixed(4)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
