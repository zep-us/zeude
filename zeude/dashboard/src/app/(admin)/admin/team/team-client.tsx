...
export default function TeamClient() {
  ...
  // React Query
  const { data: teamData, isLoading: loading, isError, refetch } = useTeamUsers(filterTeam, filterStatus, search)
  ...
  async function handleGenerateInvite() {
    ...
    try {
      const data = await generateInvite.mutateAsync({ team: inviteTeam, role: inviteRole })
  ...
  async function handleGenerateKey() {
    if (!keyUser) return
    try {
      const data = await generateKey.mutateAsync(keyUser.id)
              ...
              <TableBody>
                {users.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell>
                      <Badge variant={user.status === 'active' ? 'default' : user.status === 'deleted' ? 'outline' : 'destructive'}>
                        {user.status}
                      </Badge>
                    </TableCell>
        ...
        <DialogContent>
          ...
          {!inviteUrl ? (
            <div className="space-y-4">
              ...
              <DialogFooter>
                <Button onClick={handleGenerateInvite} disabled={!inviteTeam || generateInvite.isPending}>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="p-3 bg-destructive/10 border border-destructive/20 rounded-lg">
              <p className="text-sm text-muted-foreground">
                The user will be marked as deleted and can no longer access the service.
                Their activity history and created resources will be preserved.
              </p>
            </div>
            <DialogFooter>
