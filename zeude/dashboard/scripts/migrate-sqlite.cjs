...
function resolveDatabasePath() {
  ...
  return process.env.DATABASE_PATH || (isProduction ? '/var/lib/zeude/zeude.db' : path.join(process.cwd(), '.data', 'zeude.db'))
}
...
function ensureSqliteReady(databasePath = resolveDatabasePath()) {
  ...
  statements.forEach((sql, index) => {
    db.exec(sql)
    const insertMigration = db.prepare('INSERT OR IGNORE INTO _sqlite_migrations(name, applied_at) VALUES (?, ?)')
    insertMigration.run(`script_migration_${index + 1}`, new Date().toISOString())
  })
...
if (require.main === module) {
  const { db, databasePath } = ensureSqliteReady()
  db.close()
